...
async def process_chunk(chunk, start_idx, total_records, model_client, termination_condition):
    ...
    prompt = (
        ...
        f"以下為該批次資料:\n{chunk_data}\n\n"
        "請根據以下資料進行分析，參考每位病人的可能疾病、判斷嚴重程度，如需就醫請提供看診科別建議，若暫時不需要就醫，請提供藥品購買建議。請整合所有代理人的功能，生成一份完整且具參考價值的建議報告。"
        "其中請特別注意：\n"
        "  1. 針對 CSV 檔案中的每筆病人資料，請分析可能疾病的欄位，給出建議；\n"
        "  2. 請提供以下欄位：\n"
        "    - 嚴重程度評估\n"
        "    - 是否需立即就醫（是或否）\n"
        "    - 看診科別建議/藥品購買建議\n"
...
async def main():
    ...
    # HW1 Data set info
    chunk_size = 1000
    df = pd.read_csv('final_result.csv')['可能疾病']
    ...
    print(final_report)
    final_report.to_csv('medication_recommendation.csv', index=False, header=True)
